...
var getSnakeDirection = function getSnakeDirection() {
  return AVAILABLE_SNAKE_DIRECTIONS[Object.keys(AVAILABLE_SNAKE_DIRECTIONS)[getRandomNumber(0, Object.keys(AVAILABLE_SNAKE_DIRECTIONS).length)]];
};
...
var getCircleDiameter = function getCircleDiameter() {
  if (config.isCircleDiameterRandomPerCircle) {
    return getRandomNumber(20, config.isCircleDiameterMaxRandomPerDirection ? getRandomNumber(20, 50) : config.circleDiameterMax);
  }
...
if (config.layout === 'confetti') {
  ...
  createCircle = function createCircle() {
    ...
    var snakeDirection = void 0;var directionIncrements = void 0;
    ...
    if (topValue > window.innerHeight || topValue < 0 || leftValue > window.innerWidth || leftValue < 0) {
      ...
      directionIncrements = config.isDirectionIncrementsDifferentOnReset ? makeDirectionIncrements(config.isSnakeAngleRandomPerDirection, config.snakeAngleMax) : config.defaultDirectionIncrements;
    }

    snakeDirection = config.isDirectionDifferentPerCircle ? getSnakeDirection() : config.defaultSnakeDirection;

    directionIncrements = config.isDirectionIncrementsDifferentPerCircle ? makeDirectionIncrements(config.isSnakeAngleRandomPerDirection, config.snakeAngleMax) : config.defaultDirectionIncrements;

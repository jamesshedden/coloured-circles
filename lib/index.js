...
var getRandomGradientFromColors = function getRandomGradientFromColors(angle) {
...
if (config.layout === 'confetti') {
  ...
  var circleGradientCounter = 0;
  var circleGradient = config.defaultCircleGradient;
  var numberOfCirclesUntilCircleGradientChange = config.numberOfCirclesUntilCircleGradientChange;
  var isCircleGradientChangeEnabled = config.isCircleGradientChangeEnabled;
  ...
  createCircle = function createCircle() {
    ...
    if (isBorderRadiusChangeEnabled) {
      if (_circleBorderRadiusCounter < _numberOfCirclesUntilBorderRadiusChange) {
        ...
        _borderRadius = getRandomNumber(config.borderRadiusMinPercent, config.borderRadiusMaxPercent);
    ...
    if (config.isCircleGradientsEnabled) {
      color = Math.random() < config.circleGradientProbability ? getRandomGradientFromColors(circleGradient) : getRandomColorFromColors();
    } else {
      color = getRandomColorFromColors();
    }
  ...
  var _circleGradientCounter = 0;
  var _circleGradient = config.defaultCircleGradient;
  var _numberOfCirclesUntilCircleGradientChange = config.numberOfCirclesUntilCircleGradientChange;
  var _isCircleGradientChangeEnabled = config.isCircleGradientChangeEnabled;
  ...
  createCircle = function createCircle() {
    ...
    if (_isCircleGradientChangeEnabled) {
      if (_circleGradientCounter < _numberOfCirclesUntilCircleGradientChange) {
        _circleGradientCounter += 1;

        if (config.rotationGradualIncrementAmount) {
          _circleGradient += config.circleGradientGradualIncrementAmount;
        }
        ...
        //
        // The chance will be 1 in N where N is the total number of circles.
        _isCircleGradientChangeEnabled = Math.random() < 1 / config.totalCircles ? false : true;
      } else {
        _circleGradient = getRandomNumber(0, 360);

        if (config.isOffsetChangeCircleNumberReset) {
          _numberOfCirclesUntilCircleGradientChange = getRandomNumber(0, 20);
        }

        _circleGradientCounter = 0;
      }
    }
    ...
    if (!_isCircleGradientChangeEnabled) {
      // NOTE: If position offset changes are initially not enabled, there's a 1 in N chance
      // that they will become enabled (where N is number of total circles)
      _isCircleGradientChangeEnabled = Math.random() < 1 / config.totalCircles ? true : false;
    }
    ...
    if (_isBorderRadiusChangeEnabled) {
      if (_circleBorderRadiusCounter2 < _numberOfCirclesUntilBorderRadiusChange2) {
        ...
        _borderRadius2 = getRandomNumber(config.borderRadiusMinPercent, config.borderRadiusMaxPercent);
    ...
    if (config.isCircleGradientsEnabled) {
      color = Math.random() < config.circleGradientProbability ? getRandomGradientFromColors(_circleGradient) : getRandomColorFromColors();
    } else {
      color = getRandomColorFromColors();
    }
